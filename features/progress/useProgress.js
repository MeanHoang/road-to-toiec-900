...
export function useProgress(daySlug) {
  ...
  /**
   * Gắn sao một từ — "nghĩa thì nhớ rồi, nhưng hay quên mặt chữ".
   *
   * Cố ý TÁCH khỏi vocab known/unknown: một từ đã đánh dấu "đã biết" sẽ biến
   * khỏi bộ thẻ, nhưng vẫn cần lôi ra nhìn lại mặt chữ. Hai chuyện khác nhau
   * nên là hai trường khác nhau.
   */
  const setStar = useCallback(
    (id, on) => write((p) => ({ ...p, star: { ...p.star, [id]: Boolean(on) } })),
    [write],
  );
   ...
   *
   * KHÔNG đụng tới `star`: sao là ghi chú riêng của người học, học lại từ đầu
   * không có nghĩa là vứt hết đánh dấu đi.
   */
